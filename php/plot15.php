...
if (!file_exists("output/plot15${lang}.gif")) {
    ...
    foreach ($files as $file) {
        $part = explode(".", $file);
        $part = $part[1];
        if ($part < "20201002" || $part > "20210111") {
            continue;
        }
        if (file_exists("output/plot15${lang}.${part}.gif")) {
            continue;
        }
        ...
        foreach ($momonew as $key => $val) {
            ...
            if ($year == 2018) {
                $val[0] = str_replace(2018, 2020, $val[0]);
                if (isset($matrix[$val[0]]) && $matrix[$val[0]][1] == "") {
                    $matrix[$val[0]][1] = $val[1];
                }
            }
        }
        // CONTINUAR
        array_unshift($matrix, array("Fecha","2018","2019","2020","INE2018"));
        ...
        $gnuplot = implode("\n", array(
            ...
            "plot 'middle/plot15${lang}.${part}.csv' u 1:2 w lp ti col,\
            '' u 1:3 w lp ti col,\
            '' u 1:4 w lp ti col,\
            '' u 1:5 w l ti col",
        )) . "\n";
