...
if (!file_exists("output/plot18${lang}01.png")) {
        ...
        if ($key == 0) {
            ...
            foreach ($val as $key2 => $val2) {
                if ($key2 == 0) {
                    ...
                    if (substr($val2, -2, 2) == " p") {
                        $val2 = substr($val2, 0, -2);
                    }
                    if (substr($val2, -2, 2) == " e") {
                        $val2 = substr($val2, 0, -2);
                    }
                    $val[$key2] = $val2;
                }
    ...
    foreach ($europe as $key => $val) {
        ...
        $paises[$val[0]] = $val[0];
        if (isset($countries[$val[0]])) {
            $paises[$val[0]] = $countries[$val[0]];
        }
    }
    asort($paises);
    $años = array();
    ...
    foreach ($europe as $key => $val) {
        ...
        foreach ($val as $key2 => $val2) {
            ...
            $temp = explode("-", $europe[0][$key2]);
            if (!isset($matrix[$temp[1]][$val[0] . "-" . $temp[0]])) {
                continue;
            }
