...
if(!file_exists("output/plot25${lang}.png")) {
    ...
    foreach($ine as $key=>$val) {
        if($val[0]!=2019) continue;
        if(!isset($matrix[$val[1]])) $matrix[$val[1]]=array("total"=>"");
    ...
    $gnuplot=implode("\n",array(
        ...
        "plot 'middle/plot25${lang}.csv' u 2:xtic(1) ti col, '' u 3:xtic(1) ti col, '' u 4:xtic(1) ti col, '' u 5:xtic(1) ti col",
    ))."\n";
