...
find momo/data.* -name "data.*.csv" -type f -size 1k -delete
lbzip2 momo/data.*.csv
...
gzip -nf norway/07995.*.csv

# PORTUGAL
wget -O portugal/Dados_SICO_$(date +"%Y-%m-%d").js "https://evm.min-saude.pt/table?t=geral&s=0"; sleep 1
wget -O portugal/Dados_SICO_$(date +"%Y-%m-%d").js "https://evm.min-saude.pt/table?t=geral&s=0"
gzip -nf portugal/Dados_SICO_*.js
...
gzip -nf germany/sterbefallzahlen.*.js

# SPAIN
wget -O spain/35177.$(date +"%Y%m%d").csv "https://www.ine.es/jaxiT3/files/t/es/csv_bdsc/35177.csv?nocab=1"; sleep 1
wget -O spain/35177.$(date +"%Y%m%d").csv "https://www.ine.es/jaxiT3/files/t/es/csv_bdsc/35177.csv?nocab=1"
gzip -nf spain/35177.*.csv
